...
# 
# output_path<- '/home/norbert/IBB/skrypty/Main_Dataset_splitted/plots_extensive/'
# #```
...
##```{r table_with_proteins_count_for_top_10_percent_tAI_values_from_both_ends }
...
#prepare data
goterms_extreme <- GOterms %>% 
  filter(GOterms != "") %>% 
  separate_rows(GOterms, sep = '\\|') #I separated goterms from common column (fe. RNA|DNA|mito) to three independent rows with the same protein_ID and tAI in other columns.

goterms_extreme <- get_extreme_tAI_values(goterms_extreme, 'tAI', 10, 'percent')
...
colors <- colorRampPalette(brewer.pal((length(unique(hilo_goterms$GOterms))),"Paired"))(length(unique(hilo_goterms$GOterms)))

count_values <- hilo_goterms %>% 
    group_by(GOterms) %>% 
    summarise(count=n())
x_labels <- count_values %>% 
    mutate(label = paste(GOterms, "\nn=", count)) %>% 
    pull(label)
...
lowest_highest_boxplot <- ggplot(hilo_goterms, aes(GOterms, tAI, fill=as.factor(GOterms)))+
...
ggsave(paste0(output_path, 'highest_lowest_tAI_goterms.png'), plot=lowest_highest_boxplot, width = 8, height = 6, dpi=300)
##```
