...
# Iterating through files in the folder
  ...
  sed -i '1s/^[^ ]*/assembly_ID\tfasta_header_first_part\tprotein_id\ttitle\tNc\tGC3s\tGC\tL_aa\tNA\tAA_len_bin\ttAI/' "$output_file1"
  
done

awk '{print $1 "\t" $3 "\t" $NF}' "$output_file1" > "$output_file3"
...
sed -i '1i assembly_ID\tpath\ttAI_mean\tGC_mean\tL_aa_mean\tpearson\tcorrelation_tAI_NC_GC3' "$output_file2"
