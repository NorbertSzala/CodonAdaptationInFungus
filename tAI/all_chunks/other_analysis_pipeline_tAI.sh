...
tRNAscan_SE_files=($(find "$tRNAscan_SE_output" -type f -name "*.fna")) # to ma isc do petli ktora iteruje po wszystkich folderach. Dodaj prefix do koncowki folderu
    ...
    if [ -d "$tRNAscan_SE_folder" ]; then
        for file_path in "$tRNAscan_SE_folder"/*; do
            if [ -f "$file_path" ]; then
                ...
                #adding first part of fasta header as new column - CodonW uses only first 20 signs what can be non specific.
                #adding protein_id to output from codonw, because codonw creates column 'title' with only first 20 letters in fasta header.
                # Extract protein ID values from the corresponding file (assuming the IDs are inside the file)
                fasta_header_first_part=$(find "$cds_folder_path" -name "$prefix*" -print | head -n 1)  # Get one file matching the prefix
                if [ -n "$fasta_header_first_part" ]; then
                    # Assuming protein_id is in the form of [protein_id=...], extract them
                    fasta_header_first_part_column=$(grep '^>' "$fasta_header_first_part" | awk '{print $1}')
                    protein_id_column=$(echo "$fasta_header_first_part_column" | awk -F"_" '{print $3}')


                    # Add header for fasta header column
                    fasta_header_first_part_column="fasta_header_first_part"$'\n'"$fasta_header_first_part_column"
                    ...
                    fasta_header_line_count=$(echo "$fasta_header_first_part_column" | wc -l)

                    # Compare the number of lines to ensure they match
                    if [ "$codonw_line_count" -eq "$fasta_header_line_count" ]; then
                        echo "Line counts match. Proceeding with merging."

                        # Create a temporary file to store the merged content
                        tmp_file=$(mktemp)

#                        paste <(echo "${fasta_header_first_part_column[@]}") "$codonw_folder/codonw_$prefix" > "$tmp_file"
                        paste <(echo "${fasta_header_first_part_column[@]}") <(echo "${protein_id_column[@]}") "$codonw_folder/codonw_$prefix" > "$tmp_file"




#                        awk '{print $1 "\t" $6 "\t" $2 "\t" $3 "\t" $4 "\t" $5}' "$tmp_file" > file1.txt && mv file1.txt "$tmp_file"

                        # Move the temporary file to the final destination
                        mv "$tmp_file" "$codonw_folder/codonw_$prefix"
                    else
                        echo "Warning: Line counts in fasta header do not match. Cannot merge files."
                    fi
                else
                    echo "Error: No files found matching prefix $prefix in fasta header."
                fi
