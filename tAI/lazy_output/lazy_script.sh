...
#     process_pair "$pair" "$counter" "$output_folder"  # Uruchamianie funkcji na jednym rdzeniu
#     counter=$((counter + 1))
# done
...
for ((i=0; i<num_chunks; i++)); do
    ...
    # Uruchomienie przetwarzania chunku w tle
    echo "Przetwarzam chunk $((i+1))"
    for ((j=0; j<${#chunk[@]}; j++)); do
        process_pair "${chunk[$j]}" $((start_index + j + 1)) "$output_folder" &
    done

    wait  # Czeka na zakończenie operacji w tle dla tego chunku

    if (( (i + 1) % chunk_size == 0 )); then
