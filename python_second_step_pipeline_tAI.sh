...
#     python3 count_genes_coding_tRNA.py tRNAscan_SE_output_$prefix_inside counted_genes_coding_tRNA_$prefix_inside.txt
...
#                 python3 /home/norbert_s/tAI/count_genes_coding_tRNA.py "$file_path/tRNAscan_SE_output_$prefix" "$counted_genes_coding_tRNA_folder/counted_genes_coding_tRNA_$prefix.txt"
#                 check_error "count_genes_coding_tRNA.py for $prefix"

#                 echo " "
#                 echo "CodonM started for $prefix"
#                 perl codonM "$cds_folder_path/$prefix*" "codonM_output_$prefix"
#                 check_error "codonM for $prefix"

#                 echo " "
#                 echo "CodonW started for $prefix"
#                 codonw "$cds_folder_path/$prefix*" "$codonm_folder/codonw_output_$prefix" "$codonm_folder/codons_NC_bulk_output_$prefix" -nomenu -nowarn -silent -enc -gc -gc3s -L_aa
...
for folder_index in {1..10}; do #zmien na 10
    ...
    mkdir -p "$tAI_folder"
    echo "file path - plik? $file_path"
    echo "Prefix $prefix"
    echo "tRNAscan_SE_folder $tRNAscan_SE_folder"
    echo "counted_genes_coding_tRNA_folder $counted_genes_coding_tRNA_folder"
    echo "codonm_folder $codonm_folder"
    echo "codonw_folder $codonw_folder"
...
# codonw "$(find "$cds_folder_path" -name "$prefix*" -print -quit)" "$codonw_folder/codonw_$prefix" "$codonw_folder/bulk_$prefix" -nomenu -nowarn -silent -enc -gc -gc3s -L_aa -noblk
# codonw /db/183_fungi_mai_2022/cds/GCA_000182895.1cds_from_genomic.fna /home/norbert_s/tAI/all_chunks/output_shorted_pipeline_tAI1/codonw_folder/codonw_output_GCA_000182895 /home/norbert_s/tAI/all_chunks/output_shorted_pipeline_tAI1/codonw_folder/bulk_GCA_000182895 -nomenu -nowarn -silent -enc -gc -gc3s -L_aa


# $(find "$/db/183_fungi_mai_2022/cds" -name "$GCA_000182895*" -print -quit)